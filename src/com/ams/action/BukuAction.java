	...
	public ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			if(form.getString("subaction").length() > 0 && form.getString("subaction").equalsIgnoreCase("search")){
				...
				if (form.getString("namaBuku")!=null && form.getString("namaBuku").length()>0)
					criteria.add(Restrictions.ilike("NamaBuku", form.getString("namaBuku"), MatchMode.ANYWHERE));
				
				if (form.getString("kodeBuku")!=null && form.getString("kodeBuku").length()>0)
					criteria.add(Restrictions.ilike("KodeBuku", form.getString("kodeBuku"), MatchMode.ANYWHERE));
				
				if (form.getString("namaKategori")!=null && form.getString("namaKategori").length()>0)
					criteria.add(Restrictions.ilike("NamaKategori", form.getString("namaKategori"), MatchMode.ANYWHERE));
